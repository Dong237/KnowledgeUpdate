...
The above process is performed concurrently for each document, utilizing multithreading,
and requires a LLM api instead of a local LLM.
...
def _generate_qa_pairs_from_one_txt_file(data_path: str, data_name: str, qa_amount_per_fact: int, 
    ...
    qa_pairs_all_chunks = []
    facts = []
    for chunk in tqdm(chunking_results, desc=f"Processing chunks for {data_name}..."):
        ...
        for fact in tqdm(parsed_facts, desc=f"Processing facts for {data_name}..."):
...
def generate_qa_pairs_from_folder(data_path: str, qa_amount_per_fact: int, role_amount_per_fact: int, 
                                  chunk_size_by_token: int, json_save_dir: str):
